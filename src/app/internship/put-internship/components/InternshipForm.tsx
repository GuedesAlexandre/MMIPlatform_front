...
interface InternshipPutFormProps {
  onSubmit: SubmitHandler<Internship>;
  internshipToPut ?: Internship;
}

const InternshipPutForm: React.FC<InternshipPutFormProps> = ({ onSubmit, internshipToPut }) => {
  const methods = useForm<Internship>();

  useEffect(() => {
    methods.reset(internshipToPut);
  }, [internshipToPut, methods]);
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormMessage>
                {methods.formState.errors.type && "Ce champ est obligatoire"}
              </FormMessage>
            </FormItem>
          )}
        />
...
export default InternshipPutForm;